param(
  ...
  [int]$Limit = 1000,
  [string]$OutputCsv = (Join-Path $PSScriptRoot "..\reports\ready-validation-$CohortId-$((Get-Date).ToString('yyyy-MM-dd')).csv"),
...
WHERE pc.cohort_id = '$CohortId'
  AND COALESCE(pc.pipeline_status, pc.final_status) IN ('candidate_validation', 'routing_only_pre_validation')
